...
在 26.9.9C 的基础上，增加上游 HME 地址数量上限错误的持久化处理。收到 `-41012` 后，账号会标记为达到上限并移出生产池，后续提交直接拦截。

| 模块 | 变更 |
|------|------|
| 上游上限 | `-41012` 写入 `account_flags.alias_limit_reached`，记录原因和时间 |
| 生产池 | 达到上游上限的账号自动移出单次生产和轮询生产选择 |
| 页面提示 | 账号页、生产页显示上游地址数量上限状态 |
| 文档与测试 | 补充策略说明和上限标记回归测试 |

## 版本 26.9.9C 变更摘要

在 26.9.9B 的基础上，增加免费 5 GB 套餐账号的人工解锁入口。续费后可在生产页清除旧套餐标记，再重新选择账号加入生产池。

| 模块 | 变更 |
|------|------|
| 解锁接口 | 新增 `POST /api/production-loop/accounts/{account}/unlock`，清除账号 `free_plan` 标记 |
| 生产页 | 单次生产页和轮询生产页显示“解锁勾选”按钮；解锁后恢复选择能力，不自动启动生产 |
| 测试 | 增加人工解锁后的生产资格和选择状态回归测试 |
...
| 总量风控 | 单账号最多保有 `750` 个隐私邮箱，停用地址也计入；已有地址加在途占位达到上限时返回 `409 HME_ACCOUNT_LIMIT` |
| 轮询生产 | 满额账号自动跳过且不计失败；保存最近任务及网络报告，运行日志区分提交失败、限流、跳过和生产结果 |
| 生产前端 | 账号配额增加 `总量 x/750` 和在途数量；满额后禁用生产按钮/轮询复选框；任务状态恢复独立滚动区 |
| 邮件详情 | 纯文本正文按需写入 SQLite 缓存，列表查询不读取大字段；HTML 仅在点击 HTML 标签时实时拉取 |
| 区域兼容 | 中国区浏览器仍使用 `icloud.com.cn`，HME API 验证固定使用全局 `setup.icloud.com` |
| 测试 | 增加直连/代理切换、正文缓存、失败释放占位、749+1 并发边界、750 上限、轮询跳过和 HTTP 409 契约测试 |
...
> ### 每个账户总量最多 750 个；滚动 1 小时内最多 5 个；两次生产至少间隔 13–15 分钟

| 项 | 规定 |
|----|------|
| 范围 | **按账户分别计数**（互不影响） |
| 总量上限 | **750 个 / 账户**；停用地址仍计入，在途原子占位也预占容量 |
| 窗口 | **滚动 1 小时**（非自然整点） |
| 上限 | **5 个 / 小时 / 账户** |
...
| 常量 | `tools/rate_limit.py` → `HME_ACCOUNT_ALIAS_LIMIT = 750`、`HME_CREATE_LIMIT_PER_HOUR = 5`、`HME_CREATE_MIN/MAX_INTERVAL_MINUTES = 13/15` |
| 满额错误 | HTTP `409`，错误码 `HME_ACCOUNT_LIMIT`；这是容量上限，不返回 `Retry-After` |
| 上游满额 | Apple HME 返回 `-41012` 时写入 `account_flags.alias_limit_reached`，移出生产池；后续提交返回 `409 HME_ACCOUNT_LIMIT` |
...
| 限流 | 单账号总量 750 + 1 小时 5 个 + 13–15 分钟间隔；支持容量与配额查询 |
...
└── tools/
    ...
    ├── rate_limit.py            # 总量 750 + 1h/5 + 13–15 分钟间隔
...
| 总量容量 | 已有地址与在途任务达到 `750` 后禁用提交，并返回 `HME_ACCOUNT_LIMIT` |
...
| 满额账号 | 已有地址与在途任务达到 `750` 时禁用选择；运行期间达到上限则记为“跳过”，不计生产失败 |
| 上游满额账号 | 收到 `-41012` 后持久化为达到上限，移出轮询池并记为“跳过”；释放名额后清除该标记再重新勾选 |
...
| 表 | 用途 |
|----|------|
| `create_claims` | 创建前原子占位；防止多线程同时越过 750 总量或每小时上限 |
